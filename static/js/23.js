...
function initMap() {
    ...
    map = DG.map('map', {
        center: [55.7558, 37.6173], // Центр карты на Москву
        zoom: 11
    });
    // Добавляем маркеры для каждого причала
    for (let i = 0; i < piers.length; i++) {
        let pier = piers[i];
        let marker = DG.marker(pier.coords).addTo(map);
        // При клике на маркер устанавливаем его как начальную или конечную точку
        marker.on('click', function () {
            if (!fromMarker) {
                fromMarker = this;
                this.bindPopup(`Выбрана стартовая точка: ${pier.name}`).openPopup();
            } else if (!toMarker) {
                toMarker = this;
                this.bindPopup(`Выбрана конечная точка: ${pier.name}`).openPopup();

                // Строим маршрут после выбора второй точки
                plotRoute();
            }
        });
    }
...
function plotRoute() {
    if (fromMarker && toMarker) {
        ...
        DG.Routing.control({
            waypoints: [
                DG.latLng(fromCoords.lat, fromCoords.lng),
                DG.latLng(toCoords.lat, toCoords.lng)
            ],
...
// Проверка, является ли точка допустимым причалом
function isValidPier(coords) {
    return availablePiers.some(pier => {
        const distance = Math.sqrt(Math.pow(coords[0] - pier.coords[0], 2) + Math.pow(coords[1] - pier.coords[1], 2));
        return distance < 0.005; // Радиусное расстояние для допускаемого выбора причала
    });
}
// Инициализация карты при загрузке страницы
document.addEventListener('DOMContentLoaded', initMap);

// Функция для добавления кораблей
...
// Иконки кораблей
const boatIcons = boats.map(boat => {
    const boatPlacemark = new ymaps.Placemark(boat.coords, {
        hintContent: boat.name,
    }, {
        iconLayout: "default#image",
        iconImageHref: "../static/icons/Ship.png", // Иконка корабля
        iconImageSize: [40, 40],
        iconImageOffset: [-20, -20],
    });
    myMap.geoObjects.add(boatPlacemark);
    return boatPlacemark;
});

// Функция плавной анимации корабля
function animateBoat(boatPlacemark, routes, duration = 120000) {
    const stepsPerSegment = 200; // Количество шагов на каждый сегмент маршрута
    const totalDuration = duration / routes.length; // Время на каждый сегмент маршрута
    let currentRouteIndex = 0;

    // Функция для анимации движения между двумя точками маршрута
    function animateSegment(start, end, placemark, onComplete) {
        const deltaLat = (end[0] - start[0]) / stepsPerSegment;
        const deltaLng = (end[1] - start[1]) / stepsPerSegment;
        let currentStep = 0;
        ...
        function moveStep() {
            if (currentStep < stepsPerSegment) {
                ...
                currentStep++;
                setTimeout(moveStep, totalDuration / stepsPerSegment);
            } else {
                onComplete && onComplete(); // Завершаем анимацию для этого сегмента
            }
        }
    ...
    // Функция для запуска анимации по всем сегментам маршрута
    function startRouteAnimation() {
        if (currentRouteIndex >= routes.length) {
            currentRouteIndex = 0; // Зацикливаем маршруты
        }

        const route = routes[currentRouteIndex];
        const { from, to } = route;

        animateSegment(from, to, boatPlacemark, () => {
            currentRouteIndex++; // Переход к следующему сегменту
            startRouteAnimation(); // Рекурсивно запускаем следующую анимацию
        });
    }

    startRouteAnimation(); // Запуск анимации
}

// Запуск анимации для всех кораблей
boatIcons.forEach((placemark) => {
    animateBoat(placemark, routes);
});
