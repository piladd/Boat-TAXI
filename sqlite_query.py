...
# ЗАПРОСЫ, СВЯЗАННЫЕ С РЕГИСТРАЦИЕЙ И ВХОДОМ (RL-7)
...
insert_orders = """INSERT INTO orders (user_id, driver_id, driver_name, driver_average_raiting, pickup_location, destination, distance, boat_category, boat, start_time, end_time, total_ride_time, order_amount, payment_method, waiting_time, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"""

select_free_drivers = (
    """SELECT * FROM drivers WHERE status = 'free' AND boat_category = ?"""
)
...
insert_drivers = """INSERT INTO drivers (full_name, boat_model, boat_image, boat_category, boat_number, status, total_trips, average_rating)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?);"""
